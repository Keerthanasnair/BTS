...
def register(request):
    ...
    if(reg_p1.count() > 0):
        ...
        print(f'Name: {saved_User.username}, Mail: {saved_User.email}, Password: {saved_User.password}, Role: {saved_User.role}')
        return JsonResponse(data,safe=False)

def admin_home(request):
    total_tasks = Scripts.objects.count()
    pending_tasks = Tasks.objects.filter(status='in progress').count()
    completed_tasks = Tasks.objects.filter(status='test done').count()
...
def developer_home(request):
    if 'mail' in request.session:
        ...
        pending_scripts = Tasks.objects.filter(script_from=username, status='sent').count()

        # Query the number of scripts in progress
        in_progress_scripts = Tasks.objects.filter(script_from=username, status='in progress').count()

        # Query the number of completed scripts
        completed_scripts = Tasks.objects.filter(script_from=username, status='test done').count()
        context = {
            'username':username,
...
def tester_home(request):
    if 'mail' in request.session:
        ...
        in_progress_scripts = Tasks.objects.filter(assign_to=username, status='in progress').count()
    ...
    return render(request,'tester_home.html',{'username':username,'total_assigned_scripts':total_assigned_scripts,'in_progress_scripts':in_progress_scripts,'completed_reports':completed_reports})
...
def edit_user(request):
    ...
    edit_ob1.role = role
    print(f'id:{id}, Userame: { edit_ob1.username}, Mail: { edit_ob1.email}, PSWD: { edit_ob1.password}, Rolee: { edit_ob1.role}')

    edit_ob1.save()
    data['msg']='Successfully Updated!!..'
    return JsonResponse(data,safe=False)
    return HttpResponse("<script>alert('Successfully Updated!!..');window.location.href='/users_list/'</script>")
...
def task_assign(request):
    ...
    return render(request, 'task_assign.html', {'reports': reports})
...
def completed_task(request):
    completed_tasks = Tasks.objects.filter(status='test done')
    context = {
...
def dev_pending(request):
    if 'mail' in request.session:
        mail=request.session['mail']
        usr_obj=User.objects.get(email=mail)
        pending_tasks = Tasks.objects.filter(status='in progress',script_from=usr_obj.username)
        context = {
            'tasks': pending_tasks,      
        }
...
def tester_progress(request):
    if 'mail' in request.session:
        mail=request.session['mail']
        usr_obj=User.objects.get(email=mail)
        in_progress = Tasks.objects.filter(status='in progress',script_from=usr_obj.username)
        context = {
            'tasks': in_progress,      
        }
...
def dev_completed(request):
    if 'mail' in request.session:
        mail=request.session['mail']
        usr_obj=User.objects.get(email=mail)
        completed_tasks = Tasks.objects.filter(status='test done',script_from=usr_obj.username)
        context = {
            'tasks': completed_tasks,      
        }
...
def upload_page(request):
    if 'mail' in request.session:
        ...
        return render(request,'upload_script.html',{'username':username,'total_scripts':total_scripts})
...
def upload_script(request):
    ...
    if request.method == 'POST':
        ...
        if file:
            ...
            Notification.objects.create(user_email=email,message=f'The {file.name} script has been uploaded by {obj.username} on {dt_string}.')
...
def developer_view_status(request):
    reports = Report.objects.all()
    return render(request,'developer_view_status.html',{'reports': reports})
...
def tasks_page(request):
    if 'mail' in request.session:
        ...
        if user.role == 'tester':
            ...
            return render(request, 'tasks.html', {'tasks': tasks, 'username': user.username})
...
def accept_task(request):
    if request.method == 'POST':
        ...
        try:
            ...
            task.save()
            Notification.objects.create(user_email=request.session['mail'],message=f"The task has been accepted by {request.session['mail']}")
...
def test_script(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            print(pylint_output)  
            if requirements_met(requirements, pylint_output):  # You'll need to implement this logic
                obj.status = 'test done'
                obj.save()
                Notification.objects.create(user_email=request.session['mail'], message=f"The testing of {script_obj.filename} has been completed successfully.")
            else:
                obj.status = 'test failed'
                obj.save()
                Notification.objects.create(user_email=request.session['mail'], message=f"The testing of {script_obj.filename} has failed.")
            ...
            return JsonResponse(data,safe=False)      
            return render(request, 'report.html', {'report': pylint_output, 'filename': script_obj.filename})
...
def tester_review_script(request):
    tasks = Tasks.objects.filter(status="test done").select_related('script')  
    task_data = []
    for task in tasks:
        script = task.script
        task_data.append({
            'task': task,
            'requirements': script.requirements,  # Fetch the requirements for the script
            'pylint_report': task.report,  # Assuming the pylint report is saved in the task object
        })  
...
def completed_reports(request):
    if 'mail' in request.session:
        ...
        return render(request,'completed_reports.html',{'reports': reports})
